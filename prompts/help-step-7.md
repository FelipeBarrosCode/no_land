...
Step 7 of onboarding: The user must choose between installing WireGuard (which requires manual configuration file imports per server) or Tailscale (which is fully automated but requires an API key) for their VPN connection.