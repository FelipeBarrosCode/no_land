...
The settings panel configures Noland's automations. Vast.ai API Key automates server renting. The connection section now reflects the managed GotaTun-backed WireGuard-compatible tunnel used for desktop provisioning. Server Filters set default marketplace requirements. Moonlight Preferences control the stream's resolution, bitrate, and framerate, which directly impacts video quality and bandwidth usage.