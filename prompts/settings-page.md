...
The settings panel configures Noland's automations. Vast.ai API Key automates server renting. Tailscale API Key automates VPN setup. Connection Provider switches between WireGuard and Tailscale. Server Filters set default marketplace requirements. Moonlight Preferences control the stream's resolution, bitrate, and framerate, which directly impacts video quality and bandwidth usage.