...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if initial_state.server_preferences.template_hash.trim().is_empty() {
                initial_state.server_preferences.template_hash = config.default_template_hash.clone();
                state_changed = true;
            }
            ...
            if initial_state.sunshine.headless_edid_base64.trim().is_empty() {
            ...
            let api_enabled = std::env::var("NOLAND_API_ENABLED")
                .ok()
                .map(|value| matches!(value.as_str(), "1" | "true" | "TRUE" | "yes" | "YES"))
                .unwrap_or(false);
            if api_enabled {
                let bind =
                    std::env::var("NOLAND_API_BIND").unwrap_or_else(|_| "127.0.0.1:8787".to_string());
                let api_state = api::state::ApiState::new(context.clone(), app.handle().clone());
                tauri::async_runtime::spawn(async move {
                    match bind.parse() {
                        Ok(addr) => {
                            if let Err(error) = api::server::run(api_state, addr).await {
                                error!("api server exited: {error}");
                            }
                        }
                        Err(error) => {
                            error!("invalid NOLAND_API_BIND value '{}': {error}", bind);
                        }
                    }
                });
            }
