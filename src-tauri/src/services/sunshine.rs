...
impl SunshineService {
    pub fn render_config(&self, detected_capture: &str, detected_output: &str) -> String {
        let mut values = BTreeMap::from([
            ...
            ("audio_sink".to_string(), self.defaults.audio_sink.clone()),
        ]);
...
"#,
    ...
    async fn setup_pipewire_config(&self, remote: &RemoteExec, target_user: &str) -> AppResult<()> {
        ...
        let audio_sink = &self.defaults.audio_sink;
        let audio_monitor = format!("{audio_sink}.monitor");

        // ── Low-latency PipeWire clock configuration ──
        let pipewire_conf = r#"# Low-latency PipeWire configuration for game streaming
# Generated by Noland Connect
...
"#;
        ...
        // ── PipeWire-Pulse low-latency fragment ──
        let pulse_conf = r#"# Low-latency PulseAudio compatibility for game streaming
# Generated by Noland Connect
...
"#;
        ...
        // ── WirePlumber ALSA low-latency fragment ──
        let wireplumber_conf = r#"# Low-latency ALSA tuning for game streaming
# Generated by Noland Connect
...
"#;
        ...
        // ── Realtime limits for audio group ──
        let audio_limits = r#"# Realtime audio limits
# Generated by Noland Connect
...
"#;
        ...
        // ── PipeWire-Pulse null sink module for Sunshine ──
        let null_sink_conf = format!(
            r#"# Virtual audio sink for Sunshine
# Generated by Noland Connect

pulse.cmd = [
  {{
    cmd  = "load-module"
    args = "module-null-sink sink_name={audio_sink} sink_properties=device.description=Noland_Audio"
    flags = [ "nofail" ]
  }}
]
"#
        ...
        // Build the full setup command as one script so all steps gate on success
        let command = if target_user == "root" {
            format!(
                r#"set -e; TARGET_HOME="{target_home}"; TARGET_USER="{target_user}"; TARGET_GID="{target_gid}"; AUDIO_SINK="{audio_sink}"; AUDIO_MONITOR="{audio_monitor}"

# 1. Install required packages
if ! dpkg-query -W -f='${{Status}}' rtkit 2>/dev/null | grep -q 'install ok installed'; then
  apt-get -o DPkg::Lock::Timeout=600 update && apt-get -o DPkg::Lock::Timeout=600 install -y rtkit || true
fi

# 2. Create config directories
mkdir -p "$TARGET_HOME/.config/pipewire/pipewire.conf.d"
mkdir -p "$TARGET_HOME/.config/pipewire/pipewire-pulse.conf.d"
mkdir -p "$TARGET_HOME/.config/wireplumber/wireplumber.conf.d"

# 3. Write low-latency config fragments
cat > "$TARGET_HOME/.config/pipewire/pipewire.conf.d/99-lowlatency.conf" <<'PIPEWIRE_EOF'
{pipewire_conf}
PIPEWIRE_EOF

cat > "$TARGET_HOME/.config/pipewire/pipewire-pulse.conf.d/10-lowlatency.conf" <<'PULSE_EOF'
{pulse_conf}
PULSE_EOF

cat > "$TARGET_HOME/.config/wireplumber/wireplumber.conf.d/10-alsa-lowlatency.conf" <<'WP_EOF'
{wireplumber_conf}
WP_EOF

cat > "$TARGET_HOME/.config/pipewire/pipewire-pulse.conf.d/20-sunshine-audio.conf" <<'SINK_EOF'
{null_sink_conf}
SINK_EOF

chown -R "$TARGET_USER:$TARGET_GID" "$TARGET_HOME/.config/pipewire" "$TARGET_HOME/.config/wireplumber"

# 4. Realtime limits
cat > /etc/security/limits.d/audio.conf <<'LIMITS_EOF'
{audio_limits}
LIMITS_EOF

# 5. Audio group membership
getent group audio >/dev/null || groupadd audio
usermod -aG audio "$TARGET_USER"

# 6. CPU governor to performance
command -v cpupower >/dev/null 2>&1 && cpupower frequency-set -g performance >/dev/null 2>&1 || true

# 7. Restart PipeWire user services
...
# 8. Create null sink for Sunshine
if ! pactl list short sinks 2>/dev/null | awk '{{print $2}}' | grep -qx "$AUDIO_SINK"; then
  pactl load-module module-null-sink sink_name="$AUDIO_SINK" sink_properties=device.description=Noland_Audio rate=48000 channels=2 >/dev/null 2>&1 || true
fi

# 9. Set default sink and source
pactl set-default-sink "$AUDIO_SINK" 2>/dev/null || true
pactl set-default-source "$AUDIO_MONITOR" 2>/dev/null || true

echo AUDIO_PIPELINE_OK
"#,
                target_home = target_home,
                target_user = target_user,
                ...
                pipewire_conf = pipewire_conf,
                pulse_conf = pulse_conf,
                wireplumber_conf = wireplumber_conf,
                null_sink_conf = null_sink_conf,
...
# 2. Create config directories (as target user)
sudo -u "$TARGET_USER" mkdir -p "$TARGET_HOME/.config/pipewire/pipewire.conf.d"
sudo -u "$TARGET_USER" mkdir -p "$TARGET_HOME/.config/pipewire/pipewire-pulse.conf.d"
sudo -u "$TARGET_USER" mkdir -p "$TARGET_HOME/.config/wireplumber/wireplumber.conf.d"

# 3. Write low-latency config fragments
sudo -u "$TARGET_USER" tee "$TARGET_HOME/.config/pipewire/pipewire.conf.d/99-lowlatency.conf" > /dev/null <<'PIPEWIRE_EOF'
{pipewire_conf}
PIPEWIRE_EOF

sudo -u "$TARGET_USER" tee "$TARGET_HOME/.config/pipewire/pipewire-pulse.conf.d/10-lowlatency.conf" > /dev/null <<'PULSE_EOF'
{pulse_conf}
PULSE_EOF

sudo -u "$TARGET_USER" tee "$TARGET_HOME/.config/wireplumber/wireplumber.conf.d/10-alsa-lowlatency.conf" > /dev/null <<'WP_EOF'
{wireplumber_conf}
WP_EOF

sudo -u "$TARGET_USER" tee "$TARGET_HOME/.config/pipewire/pipewire-pulse.conf.d/20-sunshine-audio.conf" > /dev/null <<'SINK_EOF'
{null_sink_conf}
SINK_EOF
...
# 4. Realtime limits
tee /etc/security/limits.d/audio.conf > /dev/null <<'LIMITS_EOF'
{audio_limits}
LIMITS_EOF
...
# 7. Restart PipeWire user services
if [ -S "{runtime_dir}/bus" ]; then
  sudo -u "$TARGET_USER" env XDG_RUNTIME_DIR="{runtime_dir}" DBUS_SESSION_BUS_ADDRESS="unix:path={runtime_dir}/bus" systemctl --user daemon-reload 2>/dev/null || true
  sudo -u "$TARGET_USER" env XDG_RUNTIME_DIR="{runtime_dir}" DBUS_SESSION_BUS_ADDRESS="unix:path={runtime_dir}/bus" systemctl --user restart pipewire pipewire-pulse wireplumber 2>/dev/null || true
  sleep 3
fi

# 8. Create null sink for Sunshine
run_user() {{ sudo -u "$TARGET_USER" env XDG_RUNTIME_DIR="{runtime_dir}" DBUS_SESSION_BUS_ADDRESS="unix:path={runtime_dir}/bus" "$@"; }}
if ! run_user pactl list short sinks 2>/dev/null | awk '{{print $2}}' | grep -qx "$AUDIO_SINK"; then
  run_user pactl load-module module-null-sink sink_name="$AUDIO_SINK" sink_properties=device.description=Noland_Audio rate=48000 channels=2 >/dev/null 2>&1 || true
fi

# 9. Set default sink and source
run_user pactl set-default-sink "$AUDIO_SINK" 2>/dev/null || true
run_user pactl set-default-source "$AUDIO_MONITOR" 2>/dev/null || true
...
"#,
        ...
        if output.status_code != 0 {
            return Err(AppError::Provisioning(format!(
                "Failed to set up audio pipeline (exit {}): stdout: {} | stderr: {}",
                output.status_code,
                output.stdout.trim(),
                output.stderr.trim()
            )));
        ...
        if !output.stdout.contains("AUDIO_PIPELINE_OK") {
            return Err(AppError::Provisioning(format!(
                "Audio pipeline setup did not complete: stdout: {} | stderr: {}",
                output.stdout.trim(),
                output.stderr.trim()
            )));
        }
        ...
        info!(
            target_user = target_user,
            audio_sink = audio_sink,
            "Audio pipeline configured successfully"
        );
