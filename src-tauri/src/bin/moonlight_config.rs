...
use services::moonlight::{
    MoonlightCodecPreference, MoonlightConfigureOptions, MoonlightNetworkPreference, MoonlightService,
};
...
fn parse_args() -> Result<(MoonlightConfigureOptions, Option<String>, bool), AppError> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--network" => {
                let value = args.next().ok_or_else(|| AppError::InvalidInput("--network requires a value".to_string()))?;
                options.network = match value.as_str() {
                    ...
                    _ => return Err(AppError::InvalidInput("--network must be lan|wifi|remote|auto".to_string())),
                };
            }
            "--prefer-codec" => {
                let value = args.next().ok_or_else(|| AppError::InvalidInput("--prefer-codec requires a value".to_string()))?;
                options.prefer_codec = match value.as_str() {
                    ...
                    _ => return Err(AppError::InvalidInput("--prefer-codec must be auto|h264|hevc|av1".to_string())),
                };
            }
            "--max-bitrate" => {
                let value = args.next().ok_or_else(|| AppError::InvalidInput("--max-bitrate requires a value".to_string()))?;
                let parsed = value.parse::<u32>().map_err(|_| AppError::InvalidInput("--max-bitrate must be a number".to_string()))?;
                if !(500..=500_000).contains(&parsed) {
                    return Err(AppError::InvalidInput("bitrate must be between 500 and 500000 Kbps".to_string()));
                }
                options.max_bitrate = Some(parsed);
            }
            "--fps" => {
                let value = args.next().ok_or_else(|| AppError::InvalidInput("--fps requires a value".to_string()))?;
                let parsed = value.parse::<u32>().map_err(|_| AppError::InvalidInput("--fps must be a number".to_string()))?;
                if !(1..=240).contains(&parsed) {
                    return Err(AppError::InvalidInput("fps must be between 1 and 240".to_string()));
                }
                options.fps_override = Some(parsed);
            }
            "--resolution" => {
                let value = args.next().ok_or_else(|| AppError::InvalidInput("--resolution requires a value".to_string()))?;
                let (width, height) = value
                    .split_once('x')
                    .ok_or_else(|| AppError::InvalidInput("resolution must use WIDTHxHEIGHT".to_string()))?;
                let width = width.parse::<u32>().map_err(|_| AppError::InvalidInput("resolution width must be numeric".to_string()))?;
                let height = height.parse::<u32>().map_err(|_| AppError::InvalidInput("resolution height must be numeric".to_string()))?;
                if !(256..=8192).contains(&width) || !(256..=8192).contains(&height) {
                    return Err(AppError::InvalidInput("width and height must be between 256 and 8192".to_string()));
                }
                options.resolution_override = Some((width, height));
            }
            "--set" => {
                let value = args.next().ok_or_else(|| AppError::InvalidInput("--set requires key=value".to_string()))?;
                let (key, raw_value) = value
                    .split_once('=')
                    .ok_or_else(|| AppError::InvalidInput("--set requires key=value".to_string()))?;
                options
                    .set_overrides
            ...
            "--restore-backup" => {
                restore_backup = Some(
                    args.next().ok_or_else(|| AppError::InvalidInput("--restore-backup requires a file path".to_string()))?,
                );
            }
            other => {
...
async fn main() {
    ...
    println!(
        "Backup file path: {}",
        result.backup_path.as_deref().unwrap_or(if dry_run { "dry-run: no backup created" } else { "not created" })
    );
    ...
    println!(
        ...
        result.codec_support.h264,
        result.codec_support.hevc,
        result.codec_support.av1
    );
    ...
    if !result.success {
        eprintln!("Error: {}", result.error.as_deref().unwrap_or("Moonlight configuration failed."));
        process::exit(1);
    }
