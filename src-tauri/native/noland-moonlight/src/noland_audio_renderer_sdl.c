...
typedef struct nl_audio_sdl_context {
...
int nl_audio_renderer_init(nl_audio_renderer_t* renderer,
  ...
  nl_audio_sdl_context_t* ctx = calloc(1, sizeof(nl_audio_sdl_context_t));
  if (ctx == NULL) {
    return -1;
  }
...
void nl_audio_renderer_start(nl_audio_renderer_t* renderer) {
  ...
  nl_audio_sdl_context_t* ctx = (nl_audio_sdl_context_t*)renderer->platform_context;
